...
def plot_cross_distance(filepath_data, sitename, dates, cross_distance):
    ...
    for i, key in enumerate(cross_distance.keys()):
        ...
        if len(chain) == 0 or sum(idx_nan) > 0.5 * len(idx_nan):
            continue
        ...
        ax.set(
            title="Transect %s - %d points" % (key, len(chain)),
            ylabel="distance [m]",
            ylim=get_min_max_dict(cross_distance),
        )
        fig.savefig(
            os.path.join(
                filepath_data, sitename, sitename + str(key) + "_cross_distance.png"
            )
        )
...
def find_tide_peak(filepath_data, sitename, dates, tide_level, settings):
    ...
    freqs_max = [
        freqs[idx_max] - settings["delta_f"],
        freqs[idx_max] + settings["delta_f"],
    ]
    ...
    for k in range(2):
        ...
        ax.text(
            freqs[idx_peaks[k]],
            ps_tide[idx_peaks[k]] + 1,
            "%.1f d" % ((freqs[idx_peaks[k]] ** -1) / (3600 * 24)),
            ha="center",
            va="bottom",
            fontsize=8,
    ...
    fig.savefig(
        os.path.join(
            filepath_data, sitename, "high_frequency_peak_in_the_tidal_time_series.png"
        )
    )
    return freqs_max
...
def power_spectrum(t, y, freqs, idx_cut):
    ...
    model = LombScargle(
        t, y, dy=None, fit_mean=True, center_data=True, nterms=1, normalization="psd"
    )
...
def plot_spectrum_all(
    filepath_data, sitename, key, dates_rand, composite, tsall, settings_slope
):
    ...
    xt = freq_1month / np.array(
        [12, 3, 1, 20 / (days_in_year / 12), 16 / (days_in_year / 12)]
    )
    xl = ["1y", "3m", "1m", "20d", "16d"]
    # loop for plots
    ax = fig.add_subplot(111)
    ...
    ax.set(
        xticks=xt,
        xticklabels=xl,
        title="Power Spectrum of tidally-corrected time-series",
        ylabel="amplitude",
    )
    ...
    ax.axvspan(
        xmin=settings_slope["freqs_max"][0],
        xmax=settings_slope["freqs_max"][1],
        color="0.85",
    )
    ...
    fig.savefig(
        os.path.join(
            filepath_data,
            sitename,
            "power_spectrum_of_tidally_corrected_time_series_{0}.png".format(key),
        )
    )
    ...
    fig.set_tight_layout(True)
    # axis labels
    xt = 1.0 / (np.flipud(np.arange(settings_slope["n_days"] * 2, 21, 1)) * 24 * 3600)
    xl = [
        "%d d" % (_) for _ in np.flipud(np.arange(settings_slope["n_days"] * 2, 21, 1))
    ]
    # loop for plots
    ax = fig.add_subplot(111)
    ax.axvline(x=settings_slope["freqs_max"][0], ls="--", c="0.5")
    ...
    ax.set(
        ...
        xticklabels=xl,
        ylabel="amplitude",
        title="Inset into the tidal peak frequency bands",
    )
    idx_interval = np.logical_and(
        freqs >= settings_slope["freqs_max"][0], freqs <= settings_slope["freqs_max"][1]
    )
    for i, idx in enumerate(indices):
        # compute spectrum
        ps, _, _ = power_spectrum(t, tsall[idx], freqs, [])
        ax.plot(
            freqs[idx_interval], ps[idx_interval], "-", color=color_list[idx, :], lw=1
        )
    ...
    # add legend
    nc_line = lines.Line2D(
        [], [], ls="--", c="b", lw=1.5, label="non-corrected time-series"
    )
    ax.legend(handles=[nc_line], loc=2)
    fig.savefig(
        os.path.join(
            filepath_data, sitename, "non_corrected_time_series_{0}.png".format(key)
        )
    )


def integrate_power_spectrum(
    filepath_data, sitename, key, dates_rand, tsall, settings_slope
):
    ...
    ax.grid(linestyle=":", color="0.5")
    ax.set(
        title="Energy in tidal frequency band", xlabel="slope values", ylabel="energy"
    )
    ax.plot(beach_slopes, E, "-k", lw=1.5)
    cmap = cm.get_cmap("RdYlGn")
    color_list = cmap(np.linspace(0, 1, len(beach_slopes)))
    for i in range(len(beach_slopes)):
        ax.plot(beach_slopes[i], E[i], "o", ms=8, mec="k", mfc=color_list[i, :])
    ax.plot(
        beach_slopes[np.argmin(E)],
        np.min(E),
        "bo",
        ms=14,
        mfc="None",
        mew=2,
        label="%.3f" % beach_slopes[np.argmin(E)],
    )
    fig.savefig(
        os.path.join(
            filepath_data,
            sitename,
            "energy_in_tidal_frequency_band_{0}.png".format(key),
        )
    )
